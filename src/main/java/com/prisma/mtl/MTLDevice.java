    ...
    public MemorySegment newFunction(final String mslSource, final String entryPoint) {
        try (AutoreleasePool _ = AutoreleasePool.push(); Arena arena = Arena.ofConfined()) {
            ...
            if (ObjC.isNil(library)) {
                String err = errorDescription(errorOut);
                System.err.println("[prisma] Failed to compile MSL: " + err);
                Prisma.LOGGER.error("[prisma] Failed to compile MSL: {}", err);
                Prisma.hasMSLCompileError = true;
                Prisma.lastMSLError = err;
            ...
            if (ObjC.isNil(function)) {
                Prisma.LOGGER.error("[prisma] Failed to resolve MSL entry point '{}'", entryPoint);
                return MemorySegment.NULL;
            }
            return function;
        }
