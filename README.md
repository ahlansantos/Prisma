...
- **PEU (Prisma Edge-Adaptive Upscaler):** Edge-adaptive spatial upscaling running natively in MSL. Inspired by EASU (FSR 1.0), it dynamically detects edge contrast and reconstructs crisp details for practically lossless visuals at 50% native resolution, completely eliminating blur.
...
- **VXR Isolation:** Advanced built-in features like PEU (Prisma Edge-Adaptive Upscaler) and ASFW (Frame Generation) are strictly isolated to the built-in VXR Default shader. Custom shaders render perfectly at native resolution with no G-Buffer bleeding or distortion.
