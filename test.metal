...
fragment float4 prisma_deferred_fs(
              ...
              if (celestialNdotL > 0.0f && celestialDir.y > 0.001f) {
                ...
                float ditherX = fract(52.9829189f * fract(dot(in.position.xy, float2(0.06711056f, 0.00583715f)))) * 2.0f - 1.0f;
                float ditherZ = fract(52.9829189f * fract(dot(in.position.xy + float2(13.0f, 17.0f), float2(0.06711056f, 0.00583715f)))) * 2.0f - 1.0f;
              ...
              if ((isWater || isMetal || isPuddle) && u.reflectionsEnabled > 0.5f) {
                ...
                for (int b = 0; b < maxBounces; b++) {
                    ...
                    VoxelReflResult vxr = traceVoxelReflections(voxelGrid, uVoxel.gridOrigin, uVoxel.gridSize, currentRayOrigin, currentRayDir, activeSkyLight, currentSunColor, currentMoonColor, celestialDir, sunWeight, blockAtlasTex, smp, blockUvTable, bitmaskTable, uVoxel.gridSize.w, uVoxel.lights, steps, u.maxPointLights, u.reflectionPtShadows, u.reflectionDirShadows, u.vxaoInReflections, 0.0f, u.rainStrength, u.gameTime);
